...
let mountainSelect = document.querySelector("#mountains-ddl");
const mountainInfoTableBody = document.querySelector(
  "#mountain-info-table-body"
);

//loading all the categories
function loadMountainList() {
  //creating the "select one option"
  let selectOption = document.createElement("option");
  selectOption.id = "selectOption";
  selectOption.innerText = "Select a Mountain...";
  mountainSelect.appendChild(selectOption);

  for (const mountain of mountainsArray) {
    let optionList = document.createElement("option");
    optionList.textContent = mountain.name;
    mountainSelect.appendChild(optionList);
  }
}
loadMountainList();

function buildInfoRow(tbody, theInfo) {
  let row = document.createElement("tr");
  tbody.appendChild(row);

  let cell1 = document.createElement("td");
  row.appendChild(cell1);
  cell1.innerText = theInfo.name;

  let cell2 = document.createElement("td");
  row.appendChild(cell2);
  cell2.innerText = theInfo.desc;

  let cell3 = document.createElement("td");
  row.appendChild(cell3);
  cell3.innerText = `${theInfo.elevation}ft`;

  let cell4 = document.createElement("td");
  row.appendChild(cell4);
  cell4.innerText = theInfo.effort;
  ...
  if (theInfo.img) {
    ...
    imageLink.height = 335
    imageLink.width = 220
    imageLink.src = `images/${theInfo.img}`
    cell5.appendChild(imageLink);
  }
}
function loadInfoTable() {
  clearInfoTable();

  let mountainName = mountainSelect.value;
  let mountainInfo = mountainsArray.filter(function (info) {
    return info.name == mountainName;
  });
  for (const info of mountainInfo) {
    buildInfoRow(mountainInfoTableBody, info);
  }
}
function clearInfoTable() {
  mountainInfoTableBody.innerHTML = "";
}
